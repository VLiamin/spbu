...
void build(List *list, Node *&node) {
    ...
    while (check(list, number)) {
        if ((list->first->next != nullptr) && (list->first->next->frequency < node->value)) {
            ...
            substitute = node;
            while ((node->left->value < node->right->value) && (node->left->left != nullptr) &&
                   (node->right->left != nullptr)) {
                go = true;
                substitute2 = node;
                node = node->right;
...
void coding(Node *node, List *list, long code) {
    if (node == nullptr) {
        delete node;
        return;
    }
    if (node->symbol != '*')
        push(list, node->symbol, -1, code);
    coding(node->left, list, code * 10 + 1);
    coding(node->right, list, code * 10 + 0);
